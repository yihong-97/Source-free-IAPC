...
class ROIHeads(torch.nn.Module):
    ...
    def _sample_proposals(
        self, matched_idxs: torch.Tensor, matched_labels: torch.Tensor, gt_classes: torch.Tensor, scores_weight: torch.Tensor
        ...
        if has_gt:
            ...
            scores_weight = scores_weight[matched_idxs]
            # Label unmatched proposals (0 label from matcher) as background (label=num_classes)
            scores_weight[matched_labels == 0] = 0. #1.
            # Label ignore proposals (-1 label)
            scores_weight[matched_labels == -1] = 0.
        else:
            gt_classes = torch.zeros_like(matched_idxs) + self.num_classes
            scores_weight = torch.ones_like(matched_idxs)
        ...
        return sampled_idxs, gt_classes[sampled_idxs], scores_weight[sampled_idxs]
    ...
    def label_and_sample_proposals(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            sampled_idxs, gt_classes, scores_weight = self._sample_proposals(
                matched_idxs, matched_labels, targets_per_image.gt_classes, targets_per_image.scores_weight
            )
            ...
            proposals_per_image.scores_weight = scores_weight
...
class Res5ROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            ...
            pred_instances, _ = self.box_predictor.inference(predictions, proposals, net) ##### [num_insatnce, h, w, field[pred_boxes, scores, pred_class]]
