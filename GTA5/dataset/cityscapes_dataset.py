...
class cityscapesDataSet(data.Dataset):
    def __init__(self, data_root, list_path, max_iters=None, resize_size=(1024, 512), crop_size=(512, 1024), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, set='val', autoaug=False):
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(self.data_root, "gtFine/%s/%s" % (self.set, name.replace('leftImg8bit', 'gtFine_labelIds') ))
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
