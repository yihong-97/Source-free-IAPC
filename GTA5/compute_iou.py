...
def compute_mIoU(gt_dir, pred_dir, devkit_dir=''):
    ...
    for ind in range(len(gt_imgs)):
        ...
        if ind > 0 and ind % 100 == 0:
            print(('{:d} / {:d}: {:0.1f}'.format(ind, len(gt_imgs), 100*np.mean(per_class_iu(hist)))))
...
def compute_mIoU_train(gt_dir, pred_dir, devkit_dir=''):
    """
    Compute IoU given the predicted colorized images and
    """
    with open(join(devkit_dir, 'info.json'), 'r') as fp:
    ...
    label_path_list = join(devkit_dir, 'train.txt')
    gt_imgs = open(label_path_list, 'r').read().splitlines()
...
def main(args):
    if 'train' in args.pred_dir:
        compute_mIoU_train(args.gt_dir, args.pred_dir, args.devkit_dir)
    else:
